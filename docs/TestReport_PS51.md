# Resolve-Encoding 判定テスト レポート（PS51）

- 実行日時: 2026-09-10 11:47:51
...
Runtime        : DotNetRuntimeInformation { FrameworkDescription = .NET Framework 4.8.9345.0, IsDotNetFramework = True, IsCodePagesEncodingProviderRegistered = True }
Locale         : LocaleInformation { CurrentCulture = ja-JP, AnsiCodePage = 932, OemCodePage = 932, IsAnsiOemSame = True }
PowerShellHost : PowerShellHostInformation { PSVersion = 5.1.26100.9444, PSEdition = Desktop, SupportsNumericCodePageArgument = False, SupportsAnsiEncodingName = False }
```
...
生データは同フォルダーの `TestResult_PS51.csv` にあります（BOM 無し UTF-8）。
