...
#### UTF.Unknown が .NET に無いエンコーディングを返した場合（1.2.0 で明記）

UTF.Unknown は、.NET が提供していないエンコーディングを判定結果として返すことがある。
実例として、ルーマニア語のテキストに対して `iso-8859-16` を信頼度 0.8 前後で返す。
この場合の判定結果は次のとおりとする（1.1.0 の「.NET が非対応」の扱いと同じ）。

- `EncodingWebName` … UTF.Unknown が返した名前（例: `iso-8859-16`）を保存する
- `CodePage` … **-1**（判定できなかった扱い）
- `PSEncodingName` … 設定しない（null）。`UsePSName` は false

読み書き系のコマンドは、`CodePage` が -1 であるため**判定失敗**（`EncodingDetectionFailed`、非終了エラー）として報告し、
`-Encoding` による明示指定を案内する。`-EncodingFrom` の参照先の場合は `EncodingFromDetectionFailed`（終了エラー）となる。
コードページ番号が得られたうえで実行環境がそれを提供していない場合の `CodePageNotAvailable`（8 節）とは別の経路である。

UTF.Unknown 側でエンコーディングを解決できない名前は、実測で次のとおり
（UTF.Unknown 2.7.0 / net10.0 と 2.6.0 / net48。いずれも上と同じ扱いになる）。

| 名前 | net10.0 | net48 |
|---|---|---|
| `iso-8859-16`（ルーマニア語など） | 解決できない | 解決できない |
| `iso-8859-10`（北欧） | 解決できない | 解決できない |
| `viscii`（ベトナム語） | 解決できない | 解決できない |
| `euc-tw` | 解決できない | 解決できない |
| `X-ISO-10646-UCS-4-3412` / `X-ISO-10646-UCS-4-2143` | 解決できない | 解決できない |
| `utf-7` | 解決できない | 65000 に解決できる |

1.1.0 ではこの場合に `NullReferenceException` が利用者まで届いていた（1.2.0 で修正）。

この答えにはコードページが無いため、上書き経路（独自判定の東アジア旧マルチバイトをシングルバイトで覆す）の対象にもならない。
たとえば短いスペイン語の行に対して UTF.Unknown が iso-8859-10 を 0.8 以上で返しても、東アジアのカルチャーでは独自判定の結果が残る
（下の「短文での既知の限界」）。

#### 上書き経路に入る条件（1.2.0）

上書きの下限（0.55）が効くのは、**独自判定が東アジア旧マルチバイトを返したときだけ**である。
独自判定はバイト構造の妥当性を見るため、東アジアの判定がすべて拒否するバイト
（cp1251 の `я` = 0xFF、`0x80`、`0xA0` など）を含むテキストでは「判定不能」を返す。
その場合は上書き経路に入らず、UTF.Unknown の答えが**採用の下限（0.5）だけ**で決まる。

多くの言語がこちらに該当する。実測では、東アジア以外の 17 言語の通常の長さの文を東アジアのカルチャーで判定した
300 件のうち、上書き経路に入ったのは 28 件だけだった。
経路に入るかどうかは言語ではなくバイト列で決まるため、同じ言語でも文によって変わる。

#### 短文での既知の限界（1.2.0）

概ね 90 バイト以下の短いシングルバイト系テキストでは、東アジアのカルチャーで次のことが起きる。

- **誤ったエンコーディングとして救済される。** UTF.Unknown の答え自体が誤っている
  （例: アイスランド語の 58 バイトの行が ibm852 になる。KOI8-U のウクライナ語が koi8-r になる）
- **東アジア旧マルチバイトのまま残る。** UTF.Unknown の信頼度が上書きの下限に届かない、
  または UTF.Unknown が .NET に無いエンコーディングを返す
  （例: アイスランド語の 28 バイトの行が ibm852 / 0.537 で Shift_JIS のまま。KOI8-R の 47 バイトの行が 0.40 で Shift_JIS のまま。
  スペイン語の 47〜60 バイトの行が iso-8859-10 で Shift_JIS のまま）

**閾値では解決しない。** 上書きの下限と表 B の最大の間（0.51〜0.55）にあるのは 1 件だけで、
下限を下げてもその入力は誤った ibm852 で「救済」されるだけである。原因は UTF.Unknown の短文での信頼度と精度である。
通常の長さ（約 170 バイト以上）の文では、上書き経路に入った入力はすべて正しく救済される。

これらの現在の挙動は `KnownLimitTests` で固定しているが、正しい挙動ではない（UTF.Unknown の改善に気づくため）。
測定の詳細は `docs/EncodingProbe-1.2.0-調査-クロスチェック信頼度の測定.md` にある。
UTF.Unknown を更新したときは `tools/Invoke-CrossCheckMeasurement.ps1` で再測定すること。
...
日本語（`ja`）・韓国語（`ko`）も言語サブタグの完全一致で判定する。1.1.0 は前方一致だったため、
`kok`（コンカニ語）を韓国語と判定していた。
...
| コマンド | 意味 | 対象が存在しない場合 |
|---|---|---|
| `Set-ProbedContent` | 書き込み先の既存ファイルから継承 | **非終了エラー**（そのファイルだけ書き込まない） |
| `Add-ProbedContent` | 追記先から継承 | **非終了エラー**（そのファイルだけ書き込まない） |

（1.2.0 で訂正: 当初この表は「Error 終了」としていたが、8 章の表と実装は非終了エラーであり、そちらに合わせた。
書き込み先は複数になりうるため、1 件の失敗で他のファイルの処理を止めない。1.2.0 仕様書 4.3）
...
**`-LineBreak` が決めるのは、各要素の後ろに付ける改行だけである。** 要素の文字列の中に含まれる改行
（`"a`r`nb"` など）は置き換えずにそのまま書く。たとえば `"a`r`nb"` を `-LineBreak Lf` で書くと `a\r\nb\n` になり、
1 つのファイルに CRLF と LF が混ざる。これは標準の `Set-Content` / `Out-File` と同じ挙動であり、
1.2.0 で正式な仕様として明文化した（1.2.0 仕様書 4.2。実測は `docs/EncodingProbe-1.2.0-調査-Out-File挙動の実測.md` 5 章）。
ファイル内の改行を統一する用途は `Convert-ProbedContent`（1.2.0）が担う。
...
### 8.1 メッセージとヘルプの言語（1.2.0）

エラーメッセージ（`MessageCatalog`）と `Get-Help` のヘルプ（MAML）は、英語・日本語・韓国語・
繁体字中国語・簡体字中国語で提供する。言語は `CurrentUICulture` で選ぶ。

- **メッセージの言語選択は、文字エンコーディング判定のカルチャーゲートと同じ規則を使う。**
  カルチャー名をサブタグに分解し（4.4「カルチャー名の解釈と香港 Big5」）、判定のカルチャー圏をそのまま言語に写す。
  `TwoLetterISOLanguageName` や親カルチャーのチェーンは使わない（NLS と ICU で名前・親が異なるため）。
  1.1.0 では `yue`（広東語）系が英語になり、`zh_HK` は PS 7 では簡体字、PS 5.1 では繁体字になっていた
...
| 簡体字中国語 | 簡体字中国語 |
| 繁体字中国語（台湾） | 繁体字中国語 |
| 繁体字中国語（香港・マカオ・広東語） | 繁体字中国語（台湾と同じ文面） |
| それ以外 | 英語 |

- **香港・マカオの文面は台湾版と同じ内容である。**
  Microsoft は Windows の zh-HK 言語パックの提供をやめ、zh-TW を案内している。
  香港の Windows で繁体字の UI を使う場合に実際に表示されるのは台湾向けの文面であり、それに合わせた判断である。
  ヘルプは `zh-HK` と `zh-MO` のフォルダーに `zh-TW` の複製を置く
  （`zh-MO` の親は `zh-Hant` であり、`zh-HK` フォルダーには届かないため両方に置く）
- **既知の制限:**
  - `yue` 系のカルチャーでは、メッセージは繁体字になるが、ヘルプは en-US になる。
    `Get-Help` は UI カルチャーの名前と親（`yue` / `yue-Hant`）でフォルダーを探すため、中国語のフォルダーに届かない。
    Windows に広東語の UI は無いため、実害は小さい
  - PS 7 では、用字付きのカルチャー名（`zh-Hant-TW` / `zh-Hant-HK` / `zh-Hant-MO`）と `zh_HK` のヘルプが en-US になる。
    PS 5.1（NLS）はこれらを `zh-TW` などに正規化するが、PS 7（ICU）は正規化しないためである。
    メッセージはサブタグ分解で選ぶため、この影響を受けない（`docs/EncodingProbe-課題-ヘルプの用字付きカルチャー名.md`）
